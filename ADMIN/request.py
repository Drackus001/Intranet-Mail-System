...
if (D==str(1)):
	...
	cursor.execute("update user set status=1 where user_id='"+UID+"'")
	sql = """commit"""
	cursor.execute(sql)
if (DD==str(1)):
	UID=base64.b64decode(form.getvalue('UID'))
...
print "<meta http-equiv='refresh' content='5;url=request.py?sid="+base64.b64encode(str(sid))+"' />"  
...
print """                    </li>
        ...
        <div id="page-wrapper">
            <div id="page-inner">
                <div class="row">
                    <div class="col-md-12">
                        <h1 class="page-head-line">Registration Request</h1>
...
n=cursor.execute("select * from user where user_id!=('"+sname+"') and status=('"+str(0)+"')")
row=cursor.fetchall()
...
for row in cursor:
	...
	print "</td><td><a href='request.py?sid="+base64.b64encode(str(sid))+"&D="+str(1)+"&UID="+base64.b64encode(str(row[1]))+"'>Activate</a>"
	print "</td><td><a href='request.py?sid="+base64.b64encode(str(sid))+"&DD="+str(1)+"&UID="+base64.b64encode(str(row[1]))+"'>Request Delete</a></td></tr>"
