...
if (dd==str(1)):
	if(CP and NP):
		if(CP==NP):
			cursor.execute("update user set password='"+base64.b64encode(NP)+"' where user_id=('"+uid+"')")
			sql = """commit"""
			cursor.execute(sql)
			print "<meta http-equiv='refresh' content='0;url=PasswordRecover.py?sid="+base64.b64encode(str(sid))+"' />"
		else:
			val2=1
	else:
		    val = 1  
...
<div class="col-md-6 col-sm-6 col-xs-12"><div class="panel panel-danger">
                        <div class="panel-heading">
                           Password Recover"""
if(val==1):
	print"<h1 style='color:#e30000'>Enter Password.</h1>"
if(val2==1):
	print"<h1 style='color:#e30000'>Enter Same Password</h1>"
print                        """</div>
                        <div class="panel-body">
                            <form role="form" method="post" action="Password_u.py">
                                        
                                 <div class="form-group">
                                            <label>Enter Password:</label>
                                            <input class="form-control" type="password" name="NP" placeholder="Enter Your New Password.">
                                            <label>Confirm Password:</label>
                                            <input class="form-control" type="password" name="CP" placeholder="confirm Password">"""
print"<input type='text' name='num' value='"+str(1)+"' hidden/><input type='text' name='sid' value='"+base64.b64encode(str(sid))+"' hidden/><input type='text' name='uid' value='"+base64.b64encode(str(uid))+"' hidden/>"
...
print "<button type='submit' class='btn btn-danger'>Change </button>"
