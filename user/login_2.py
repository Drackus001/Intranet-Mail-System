...
uname =base64.b64encode(form.getvalue('uname'))
password =base64.b64encode(form.getvalue('pass'))
stu=0
...
print "</head>"
print "<body>"
cursor.execute("select Account_type,status from user where user_id=('"+uname+"') and Password=('"+password+"')")
row=cursor.fetchone()
for row in cursor:
        acc=row[0]
        stu=row[1]
