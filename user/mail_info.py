...
for row in cursor:
	sname=row[0]
if(rs==str(1)):
	a="update mailcon set status=1 where no='"+str(no)+"'"
	v=cursor.execute(a)
	sql = """commit"""
	cursor.execute(sql)
...
print """                    </li>
        ...
        <div id="page-wrapper">
            <div id="page-inner">
                <div class="row">
                    <div class="col-md-12">
                        <h1 class="page-head-line">MAIL_INFO.</h1>
...
for row in cursor:
	...
	sm = form.getvalue('sm')
	if(sm==str(1)):
		print "<tr><td>Reciver ID.</td><td>",row[3]
	else:
		print "<tr><td>Sender ID.</td><td>",row[7]
		
	
	print "<tr><td >Subject</td><td>",row[2],"<tr><td colspan='2'>Message:-</td></tr></tr>"
	print "<table><tr><td><textarea style='margin: 0px; height: 346px; width: 953px;'disabled='true' >",base64.b64decode(row[1]),"</textarea></td></tr></table>"
	if(sm==str(1)):
		print "<a href='forwardmail.py?sid="+base64.b64encode(str(sid))+"&no="+str(no)+"'><button type='submit' class='btn btn-danger'>forward</button></a>"
	else:
		print "<a href='reply.py?sid="+base64.b64encode(str(sid))+"&no="+str(no)+"'><button type='submit' class='btn btn-danger'>Reply</button></a>"
		print "<a href='forwardmail.py?sid="+base64.b64encode(str(sid))+"&no="+str(no)+"'><button type='submit' class='btn btn-danger'>forward</button></a>"
		print "<a href='Label.py?sid="+base64.b64encode(str(sid))+"&no="+str(no)+"&al="+str(1)+"'><button type='submit' class='btn btn-danger'>Add into Label</button></a>"
