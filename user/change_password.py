...
a="select password from user where user_id='"+str(sname)+"'"
v=cursor.execute(a)
row=cursor.fetchall()

for row in cursor:
	PASS=base64.b64decode(row[0])
val=0
val2=0
val3=0
...
if (dd==str(1)):
	if(CP and (NP and OP)):
		if(OP==PASS):
			if(CP==NP):
				cursor.execute("update user set password='"+base64.b64encode(NP)+"' where user_id=('"+sname+"')")
				sql = """commit"""
				cursor.execute(sql)
...
<div class="col-md-6 col-sm-6 col-xs-12"><div class="panel panel-danger">
                        <div class="panel-heading">
                           Password Recover"""
if(val==1):
	print"<h1 style='color:#e30000'>Enter all Details.</h1>"
if(val3==1):
        print"<h1 style='color:#e30000'>Enter valid old password</h1>"
if(val2==1):
	print"<h1 style='color:#e30000'>Enter Same Password</h1>"
print                        """</div>
                        <div class="panel-body">
                            <form role="form" method="post" action="change_password.py">
                                        
                                 <div class="form-group">
                                 <label>Enter Old Password:</label>
                                            <input class="form-control" type="password" name="OP" placeholder="Enter Your New Password.">
